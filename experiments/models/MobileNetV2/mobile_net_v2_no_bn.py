...
def inverted_residual_block(input_tensor, multiplier, output_channels,s, add, stage,block, chkpt):
...
def create_mobilenetV2():
  ...
  with sg.Graph(name="mobilenetV2_no_bn_smv", backend="SMV") as graph:
    ...
    #stage 1 
    x= inverted_residual_block(x, 1, 16, 1, False, 1, '1', False)
    #stage 2
    x = inverted_residual_block(x, 6, 24, 2, False, 2, '1', False)
    #x = inverted_residual_block(x, 6, 24, 1, True, 2,"2", False)
    #stage 3
    x = inverted_residual_block(x, 6, 32, 2, False, 3, '1', False)
    x = inverted_residual_block(x, 6, 32, 1, True,3, '2', True)
    #x = inverted_residual_block(x, 6, 32, 1, True, 3, '3', True)
    #stage 4 
    x = inverted_residual_block(x, 6, 64, 2, False, 4, '1', False)
    #x = inverted_residual_block(x, 6, 64, 1, True, 4, '2', False)
    #x = inverted_residual_block(x, 6, 64, 1, True, 4, '3', False)
    #x = inverted_residual_block(x, 6, 64, 1, True, 4, '4', False)
    #stage 5 
    x = inverted_residual_block(x, 6, 96, 1, False, 5, '1', False)
    x = inverted_residual_block(x, 6, 96, 1, True, 5, '2', False)
    #x = inverted_residual_block(x, 6, 96, 1, True, 5, '3', False)
    #stage 6
    x = inverted_residual_block(x, 6, 160, 2, False, 6, '1', True)
    #x = inverted_residual_block(x, 6, 160, 1, True, 6, '2', False)
    #x = inverted_residual_block(x, 6, 160, 1, True, 6, '3', False)
    #stage 7
    x = inverted_residual_block(x, 6, 320, 1, False, 7, '1', False)
    #print (x._tensor_data[0,2,3,2])
    ...
    x = sg.nn.convolution(x,conv8_tensor, stride = [1,1], padding="valid",name = "conv8_chkpt")
    '''
    x = sg.nn.batch_norm(
