...
${gem5_dir}/build/X86/gem5.debug \
  --outdir=${cfg_home}/outputs_LRU_hmmr \
  ...
  --l3-repl-policy=LRURP \
  --fast-forward=10000000000 \
  --accel_cfg_file=gem5.cfg \
  -c ${bmk_dir}/smaug \
  -o "mobilenetV2_no_bn_smv_topo.pbtxt mobilenetV2_no_bn_smv_params.pb --sample-num=1 --sample-level=very_high --gem5=true --num-accels=1 --debug-level=0 --iterations=1" \
