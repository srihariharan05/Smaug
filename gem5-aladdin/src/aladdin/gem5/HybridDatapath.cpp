...
void HybridDatapath::stepExecutingQueue() {
  ...
  while (it != executingQueue.end()) {
    ...
    if (node_int->is_memory_op() || node_int->is_host_mem_op()) {
      MemoryOpType type;
      try {
        type = getMemoryOpType(node_int);
      } catch (IllegalHostMemoryAccessException& e) {
        fatal(e.what());
      }
      switch (type) {
        case Register:
          op_satisfied = handleRegisterMemoryOp(node_int);
          break;
        case Scratchpad:
          op_satisfied = handleSpadMemoryOp(node_int);
          break;
        case Cache:
          op_satisfied = handleCacheMemoryOp(node_int);
          break;
        case Dma:
          op_satisfied = handleDmaMemoryOp(node_int);
          break;
        case ReadyBits:
          op_satisfied = handleReadyBitAccess(node_int);
          break;
        case ACP:
          op_satisfied = handleAcpMemoryOp(node_int);
          break;
        default:
    ...
    } else if (node_int->is_multicycle_op()) {
      unsigned node_id = node_int->get_node_id();
      if (inflight_multicycle_nodes.find(node_id) ==
          inflight_multicycle_nodes.end()) {
        inflight_multicycle_nodes[node_id] = node_int->get_multicycle_latency();
        markNodeStarted(node_int);
      } else {
      ...
      // Not a memory/fp operation node_int, so it can be completed in one cycle.
      markNodeStarted(node_int);
      markNodeCompleted(it, index);
      op_satisfied = true;
    }
