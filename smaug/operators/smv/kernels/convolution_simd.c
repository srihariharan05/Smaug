            ...
            for (int kern_col = 0; kern_col < kern_col_sample;
                ...
                for (int ifmap_iters = 0; ifmap_iters < chan_block_sample;
                    ...
                    for ( int pe_iter = 0; pe_iter < NUM_PE_INSTS; pe_iter++){
                        for ( int mac_iter = 0; mac_iter < NUM_MACC_INSTS; mac_iter ++){
                            kernel_reg[pe_iter][mac_iter] = zero;
                        }
                        accum_vec_reg[pe_iter] = zero;
                        accum_reg[pe_iter] = 0.0;
                        results_buffer[pe_iter] =0.0;
                    }
                    /*
                    #if ( ACC_256_GOPS ==1 )
                        v8fp_t kernel_reg[NUM_PE_INSTS][NUM_MACC_INSTS] = {
                    ...
                    */
                    ...
                    for (int out_row = 0; out_row < output_row_sample;
                        ...
                        for (int out_col = 0; out_col < output_col_sample;
                            ...
                            if ( start_from_zero){
                                for ( int pe_iter =0; pe_iter < NUM_PE_INSTS; pe_iter++){
                                    results_buffer[pe_iter] = 0;
                                }
                            }
                            else
                                results_buffer = _result[out_i][out_j][ofmap_iters];
