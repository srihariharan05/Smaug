...
void smv_matrix_multiply_transpose_nc_vec_fxp(float16* host_a,
    ...
    VEC_ARRAY_2D(v8fp_t, _b, b, b_width + b_pad);
        ...
        v8fp_t partial_sums;
    #elif(ACC_1_TOPS == 1)
        VEC_ARRAY_2D(v32fp_t, _results, results, (results_width + results_pad)/RESULT_ALIGNMENT);
        v32fp_t partial_sums;
    #elif (ACC_32_TOPS ==1)
        VEC_ARRAY_2D(v256fp_t, _results, results, (results_width + results_pad)/RESULT_ALIGNMENT);
        v256fp_t partial_sums;
    #endif
    ...
    for (int a_act = 0; a_act < a_height; a_act++) {
        ...
        for (int b_row = 0; b_row < b_height; b_row += NUM_PE_INSTS) {
            ...
            if (b_row % NUM_PE_INSTS == 0) {
                if (accumulate) {
                    partial_sums = _results[a_act][(result_start + b_row) /
                                                   NUM_PE_INSTS];
                } else {
                    for ( int pe_iter =0; pe_iter < NUM_PE_INSTS; pe_iter++){
                        partial_sums[pe_iter] = 0.0;
                    }
                }
            }
            ...
            for (int b_col = 0; b_col < b_col_sample; b_col += NUM_MACC_INSTS) {
                ...
                float partial_sums_inner[NUM_PE_INSTS];
                for ( int pe_iter =0; pe_iter<NUM_PE_INSTS; pe_iter++){
                    partial_sums_inner[pe_iter] = 0.0;
                }
            ...
            if (next_b_row % NUM_PE_INSTS == 0 || next_b_row >= b_height) {
                _results[a_act][(result_start + b_row) / NUM_PE_INSTS] =
                        partial_sums;
            }
        }
    }
