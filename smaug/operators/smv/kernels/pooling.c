...
void smv_maxpooling_nhwc_vec_fxp(float16* host_inputs,
    ...
    for (int row = 0; row < input_row_sample; row += row_stride) {
        ...
        for (int col = 0; col < input_col_sample; col += col_stride) {
            maxpool_chan_grp: //this is the no of max pooling hardwares present in the acc - set the unrolling factor of this loop to equal the no of pooling units in the hw
