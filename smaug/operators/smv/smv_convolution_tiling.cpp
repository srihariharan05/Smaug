...
TilingConfig TilingOptimizer::computeBasicTileShapes(SmvConvolutionOp* op) {
    ...
    volatile int output_ch =0;
    volatile int output_h =0;
    volatile int output_tile_size =0;
    volatile int output_w = 0;
    volatile int output_n = 0; 
    volatile int pad_h =0; volatile int pad_w =0; volatile int pad_n =0; volatile int pad_c =0; 
    // Fill in outputs.
        ...
        for (int c = minChannels; c <= weightsShape[0]; c += kNumPEs) {
            ...
            if (needsHwiseTiling(outputTilingDims)) {
                int padding = op->getPadding() == SamePadding
                                      ? FRAC_CEIL(config.weights[1] - 1, 1) //2)
                                      : 0;
                ...
                output_ch = config.outputs[3];
                output_h = config.outputs[1];
                output_w = config.outputs[2];
                output_n = config.outputs[0];
                pad_h = config.outputs.getPadding(1); pad_w = config.outputs.getPadding(2);
                pad_n = config.outputs.getPadding(0); pad_c = config.outputs.getPadding(3);
            ...
            output_tile_size = config.outputs.storageSize();
            if ( output_tile_size <= maxTileSize) {
