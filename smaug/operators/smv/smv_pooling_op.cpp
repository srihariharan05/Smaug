...
#if (ACC_256_GOPS ==1) 
    const int kVectorSize = 64; //8;
#elif (ACC_1_TOPS ==1 )
    const int kVectorSize = 128;
#elif (ACC_32_TOPS ==1)
    const int kVectorSize = 2048; 
#endif
}  // namespace pool
}  // namespace smv
        ...
        for (int H = 0; H < inputRowTiles; H++) {
            for (int W = 0; W < inputColTiles; W++) {
                ...
                while (iC < inputChanTiles && oC < outputChanTiles) {
                    ...
                    //tiledTensors[1].getTile(outputTileIdx)->tensor->allocateStorage(DataType::Float16);
                    inputs[inputTileIdx]->allocateStorage(Float16);
                    outputs[outputTileIdx]->allocateStorage(Float16);
