...
void runX(UnaryOp<SmvBackend>* op, TiledTensor& inputs, TiledTensor& outputs) {
    ...
    for (int i = 0; i < inputs.size(); i++) {
        ...
        //inputs.getTile(i)->tensor->allocateStorage(DataType::Float16);
        //outputs.getTile(i)->tensor->allocateStorage(DataType::Float16);
        inputs[i]->allocateStorage(Float16);
        outputs[i]->allocateStorage(Float16);
...
std::array<TiledTensor, 2> doTiling(UnaryOp<SmvBackend>* op, bool copyData) {
    ...
    stat_num_of_tiles += tiledInputs.size() + tiledOutputs.size();
